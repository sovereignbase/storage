...
class MemoryCache {
  ...
  async put(request: Request, response: Response): Promise<void> {
    ...
    this.entries.set(request.url, retained)
...
describe('object storage', () => {
  it('stores encrypted bytes and loads the decoded object from cache', async () => {
    ...
    expect(cache.puts[0].response.status).toBe(200)
    ...
    expect(new Uint8Array(await cache.puts[0].response.arrayBuffer())).toEqual(
      stored
    )
  ...
  it('does not cache or call back for a non-successful network response', async () => {
  ...
  it.each([
    ['an unparsable URL', ID, 'not a URL/'],
    ['a non-canonical URL', ID, 'http://objects.example/'],
  ])('ignores %s', async (_label, id, host) => {
  ...
  it('does not await callback-owned asynchronous work', async () => {
  ...
  it('rejects when a callback throws synchronously', async () => {
